...
class TestCoordinatorPlayerFinder:
    """Test coordinator player finder callbacks for multiroom grouping."""
    ...
    @pytest.fixture
    def coordinator(self, hass, mock_player):
        """Create a WiiMCoordinator instance."""
        entry = MockConfigEntry(
            domain=DOMAIN,
            data=MOCK_CONFIG,
    ...
    def test_find_player_by_ip(self, hass, coordinator, mock_player):
        """Test _find_player finds player by IP address."""
        # Register coordinator in hass.data
        hass.data.setdefault(DOMAIN, {})[coordinator.entry.entry_id] = {
            "coordinator": coordinator,
        }

        with patch.object(hass.config_entries, "async_entries", return_value=[coordinator.entry]):
            found = coordinator._find_player("192.168.1.100")
            assert found == mock_player

    def test_find_player_by_uuid(self, hass, coordinator, mock_player):
        """Test _find_player finds player by UUID."""
        # Register coordinator in hass.data
        hass.data.setdefault(DOMAIN, {})[coordinator.entry.entry_id] = {
            "coordinator": coordinator,
        }

        with patch.object(hass.config_entries, "async_entry_for_domain_unique_id", return_value=coordinator.entry):
            # UUID with prefix
            found = coordinator._find_player("uuid:test-uuid-123")
            assert found == mock_player

    def test_find_player_by_uuid_without_prefix(self, hass, coordinator, mock_player):
        """Test _find_player finds player by UUID without prefix."""
        hass.data.setdefault(DOMAIN, {})[coordinator.entry.entry_id] = {
            "coordinator": coordinator,
        }
        ...
        with patch.object(hass.config_entries, "async_entry_for_domain_unique_id", return_value=coordinator.entry):
            # UUID without prefix
            found = coordinator._find_player("test-uuid-123")
            assert found == mock_player

    def test_find_player_not_found(self, hass, coordinator):
        """Test _find_player returns None when player not found."""
        ...
        with (
            ...
            found = coordinator._find_player("192.168.1.200")
            assert found is None

    def test_get_all_players_returns_all(self, hass, coordinator, mock_player):
        """Test _get_all_players returns all registered players."""
        # Create second coordinator/player
        mock_player2 = MagicMock(spec=Player)
        mock_player2.host = "192.168.1.101"

        mock_coordinator2 = MagicMock()
        mock_coordinator2.player = mock_player2
        ...
        hass.data.setdefault(DOMAIN, {})
        hass.data[DOMAIN][coordinator.entry.entry_id] = {"coordinator": coordinator}
        hass.data[DOMAIN][entry2.entry_id] = {"coordinator": mock_coordinator2}

        with patch.object(hass.config_entries, "async_entries", return_value=[coordinator.entry, entry2]):
            players = coordinator._get_all_players()

            assert len(players) == 2
            assert mock_player in players
            assert mock_player2 in players

    def test_get_all_players_empty(self, hass, coordinator):
        """Test _get_all_players returns empty list when no coordinators."""
        hass.data.setdefault(DOMAIN, {})
