...
@pytest.fixture
def mock_group_speaker(mock_group_coordinator, mock_config_entry):
    """Create a mock speaker that is a master with slaves."""
    speaker = MagicMock(spec=Speaker)
    speaker.coordinator = mock_group_coordinator
...
class TestWiiMGroupMediaPlayerBasic:
    ...
    def test_initialization(self, mock_group_speaker):
        ...
        assert entity.speaker == mock_group_speaker

    def test_name_property(self, mock_group_speaker):
    ...
    def test_available_when_master_with_slaves(self, mock_group_speaker, mock_master_player):
    ...
    def test_not_available_when_solo(self, mock_group_speaker, mock_master_player):
    ...
    def test_not_available_when_slave(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerVolume:
    ...
    def test_volume_level_returns_group_max(self, mock_group_speaker, mock_master_player):
    ...
    def test_volume_level_returns_none_when_unavailable(self, mock_group_speaker):
    ...
    def test_volume_step_reads_from_config(self, mock_group_speaker, mock_config_entry):
    ...
    def test_volume_step_defaults_when_not_configured(self, mock_group_speaker):
        """Test volume step defaults when not configured."""
        mock_group_speaker.config_entry.options = {}

        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        assert entity.volume_step == DEFAULT_VOLUME_STEP

    async def test_set_volume_level(self, mock_group_speaker, mock_master_player):
        ...
        mock_group_speaker.coordinator.async_request_refresh.assert_called_once()

    async def test_set_volume_level_handles_error(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerMute:
    ...
    def test_is_volume_muted_returns_true_when_all_muted(self, mock_group_speaker, mock_master_player):
    ...
    def test_is_volume_muted_returns_false_when_any_unmuted(self, mock_group_speaker, mock_master_player):
    ...
    async def test_mute_volume(self, mock_group_speaker, mock_master_player):
    ...
    async def test_unmute_volume(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerPlayback:
    ...
    async def test_media_play(self, mock_group_speaker, mock_master_player):
        """Test play command."""
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
    ...
    async def test_media_pause(self, mock_group_speaker, mock_master_player):
        """Test pause command."""
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
    ...
    async def test_media_stop(self, mock_group_speaker, mock_master_player):
        """Test stop command."""
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
    ...
    async def test_media_next_track(self, mock_group_speaker, mock_master_player):
    ...
    async def test_media_previous_track(self, mock_group_speaker, mock_master_player):
    ...
    async def test_playback_handles_connection_error(self, mock_group_speaker, mock_master_player):
        """Test playback commands handle connection errors gracefully."""

        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        mock_group_speaker.coordinator.player.play = AsyncMock(side_effect=WiiMConnectionError("Connection lost"))
    ...
    async def test_playback_handles_timeout_error(self, mock_group_speaker, mock_master_player):
        """Test playback commands handle timeout errors gracefully."""

        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        mock_group_speaker.coordinator.player.play = AsyncMock(side_effect=WiiMTimeoutError("Timeout"))

        # Playback methods raise generic HomeAssistantError (not "temporarily unreachable" like volume/mute)
        with pytest.raises(HomeAssistantError, match="Failed to play"):
            await entity.async_media_play()

    async def test_playback_handles_other_errors(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerState:
    """Test group media player state."""

    def test_state_playing(self, mock_group_speaker, mock_master_player):
    ...
    def test_state_paused(self, mock_group_speaker, mock_master_player):
        """Test state when paused."""
        mock_master_player.play_state = "pause"

        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        assert entity.state == MediaPlayerState.PAUSED

    def test_state_idle(self, mock_group_speaker, mock_master_player):
    ...
    def test_state_none_when_unavailable(self, mock_group_speaker):
...
class TestWiiMGroupMediaPlayerMediaInfo:
    ...
    def test_media_title(self, mock_group_speaker, mock_master_player):
    ...
    def test_media_artist(self, mock_group_speaker, mock_master_player):
    ...
    def test_media_album_name(self, mock_group_speaker, mock_master_player):
    ...
    def test_media_image_url(self, mock_group_speaker, mock_master_player):
    ...
    def test_media_duration(self, mock_group_speaker, mock_master_player):
    ...
    def test_media_position(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerShuffleRepeat:
    ...
    def test_shuffle_supported_returns_true(self, mock_group_speaker, mock_master_player):
    ...
    def test_shuffle_supported_returns_false_when_not_supported(self, mock_group_speaker, mock_master_player):
    ...
    def test_shuffle_returns_true(self, mock_group_speaker, mock_master_player):
        """Test shuffle property returns True when enabled."""
        mock_master_player.shuffle = True
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        assert entity.shuffle is True

    def test_shuffle_returns_false(self, mock_group_speaker, mock_master_player):
        """Test shuffle property returns False when disabled."""
        mock_master_player.shuffle = False
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        assert entity.shuffle is False

    def test_shuffle_returns_none_when_player_missing(self, mock_group_speaker):
    ...
    @pytest.mark.asyncio
    async def test_set_shuffle_enables(self, mock_group_speaker, mock_master_player):
    ...
    @pytest.mark.asyncio
    async def test_set_shuffle_handles_error(self, mock_group_speaker, mock_master_player):
    ...
    def test_repeat_supported_returns_true(self, mock_group_speaker, mock_master_player):
    ...
    def test_repeat_returns_off(self, mock_group_speaker, mock_master_player):
    ...
    def test_repeat_returns_one(self, mock_group_speaker, mock_master_player):
    ...
    def test_repeat_returns_all(self, mock_group_speaker, mock_master_player):
    ...
    @pytest.mark.asyncio
    async def test_set_repeat_all(self, mock_group_speaker, mock_master_player):
    ...
    @pytest.mark.asyncio
    async def test_set_repeat_handles_error(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerPlayMedia:
    ...
    @pytest.mark.asyncio
    async def test_play_media_delegates_to_master(self, mock_group_speaker, mock_master_player):
    ...
    @pytest.mark.asyncio
    async def test_play_media_handles_error(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerMediaContent:
    ...
    def test_media_content_type(self, mock_group_speaker):
...
class TestWiiMGroupMediaPlayerImageHandling:
    ...
    def test_media_image_url_returns_none_when_unavailable(self, mock_group_speaker):
    ...
    def test_media_image_url_returns_placeholder_when_no_url(self, mock_group_speaker, mock_master_player):
    ...
    def test_media_image_remotely_accessible(self, mock_group_speaker):
        """Test media_image_remotely_accessible returns False."""
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        assert entity.media_image_remotely_accessible is False

    @pytest.mark.asyncio
    async def test_get_media_image_returns_cover_art(self, mock_group_speaker, mock_master_player):
    ...
    @pytest.mark.asyncio
    async def test_get_media_image_returns_none_when_unavailable(self, mock_group_speaker):
    ...
    @pytest.mark.asyncio
    async def test_get_media_image_handles_missing_fetch_method(self, mock_group_speaker, mock_master_player):
    ...
    @pytest.mark.asyncio
    async def test_get_media_image_handles_exception(self, mock_group_speaker, mock_master_player):
...
class TestWiiMGroupMediaPlayerExtraState:
    ...
    def test_extra_state_attributes_when_available(self, mock_group_speaker, mock_master_player):
    ...
    def test_extra_state_attributes_when_unavailable(self, mock_group_speaker):
...
class TestWiiMGroupMediaPlayerSupportedFeatures:
    ...
    def test_supported_features_when_unavailable(self, mock_group_speaker):
...
class TestWiiMGroupMediaPlayerStateEdgeCases:
    """Test group media player state edge cases."""

    def test_state_derives_playing_from_playing(self, mock_group_speaker, mock_master_player):
        """Test state derives PLAYING from 'playing'."""
        from homeassistant.components.media_player import MediaPlayerState

        mock_master_player.play_state = "playing"
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        entity._attr_state = None

        assert entity.state == MediaPlayerState.PLAYING

    def test_state_derives_playing_from_load(self, mock_group_speaker, mock_master_player):
        """Test state derives PLAYING from 'load'."""
        from homeassistant.components.media_player import MediaPlayerState

        mock_master_player.play_state = "load"
        entity = WiiMGroupMediaPlayer(mock_group_speaker)
        entity._attr_state = None

        assert entity.state == MediaPlayerState.PLAYING

    def test_state_uses_attr_state_when_set(self, mock_group_speaker):
    ...
    def test_state_returns_none_when_unavailable(self, mock_group_speaker):
...
class TestWiiMGroupMediaPlayerUpdatePosition:
    ...
    def test_update_position_handles_missing_player(self, mock_group_speaker):
