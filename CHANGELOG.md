...
- **Dependency**: `pywiim` **2.3.3** (`manifest.json`, `pywiim-version.txt`, `requirements_dev.txt`), up from 2.3.2. Delivers PlayQueue auto-advance and `queue_position`/`queue_count` overlay ([Issue #268](https://github.com/mjcumming/wiim/issues/268)).
...
- **CI/CD**: Fixed GitHub Actions test suite failing with pycares import error
  - Added version constraints file (`.github/workflows/constraints.txt`) with pycares (4.4.0) and aiodns (3.2.0) for Python 3.13 compatibility
  - Updated workflow to use constraint file: `pip install -c .github/workflows/constraints.txt`
...
- **Dependency Update**: Updated minimum `pywiim` library version to 2.1.27
...
- **Documentation**: Fixed diagnostic script instructions in `scripts/DIAGNOSTIC-GUIDE.md` and `scripts/README.md`
  - Corrected instructions that incorrectly assumed Python was available in Home Assistant Terminal add-on
  - Updated to use SSH access with `docker exec` to run scripts in the Home Assistant container
  - Fixed script URL to correct path: `pywiim/cli/monitor_cli.py`
  - Added clear warnings about Terminal add-on limitations
  - Provided working alternatives for running diagnostic tools
