...
def _capabilities_prefer_http(capabilities: dict[str, Any] | None) -> bool:
    """Return True when device capabilities prefer HTTP over HTTPS.

    ``protocol_priority`` is an ordered list/tuple (e.g. ``["http", "https"]``);
    the first entry is the preferred protocol. Returns False when the priority is
    unknown so we never drop a cached endpoint without positive evidence that the
    device is HTTP-first.
    """
    if not capabilities:
        return False
    priority = capabilities.get("protocol_priority")
    if not priority:
        return False
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if not cached_endpoint and _capabilities_require_https(cached_capabilities):
        ...
        cached_endpoint = f"https://{host}:443"
        port = 443
        protocol = "https"
        _LOGGER.info(
            "Restoring HTTPS endpoint for %s from cached HTTPS-only capabilities",
            host,
        )
        hass.config_entries.async_update_entry(
            entry,
            data={**entry.data, "endpoint": cached_endpoint},
        )
