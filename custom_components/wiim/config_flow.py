...
class WiiMConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
        ...
        async def _on_device(device):
            ...
            if is_valid or device_uuid:
                ...
                if unique_id in known_uuids:
                    _LOGGER.debug(
                        "Skipping already configured device with UUID %s at host %s",
                        unique_id,
                        host,
                    )
                    return
    ...
    async def async_step_ssdp(
        ...
        ssdp_location = (
            discovery_info.ssdp_location.lower() if discovery_info.ssdp_location else ""
        )
        ssdp_server = (
            discovery_info.ssdp_server.lower() if discovery_info.ssdp_server else ""
        )
        ...
        if is_valid and device_uuid:
            ...
            # Store data for discovery confirmation (including SSDP location for UPnP)
            self.data = {
                CONF_HOST: host,
                "name": device_name,
                "ssdp_info": {"location": discovery_info.ssdp_location}
